
import * as React from 'react';
import Icone from 'react-native-vector-icons/Feather'
import { View, FlatList, Text, Image, TouchableOpacity } from 'react-native';
import styles from './styles'
import {AMB} from '../../../params'
import logoImg from '../../assets/img/logo.png'
import {useNavigation, useRoute} from '@react-navigation/native'
//import Icon from 'react-native-vector-icons/FontAwesome'
import Moment from 'moment'

export function detailEventos() {
  const navigation = useNavigation();
  const route = useRoute();
  const eventos = route.params.eventos

  function navigateToDetail (){
    navigation.navigate('Eventos')

  }
  return (
  <View style={styles.container}>
    <View style={styles.header}>
      <Image source={logoImg} />
     
    </View>
    
    <View style={styles.body}>
      <Text style={styles.title}>{eventos.Titulo}</Text>
    
            <View style={styles.eventos}>
              
              <Text style={styles.eventosDC}>Data:  <Text style={styles.dados}>{eventos.Data_do_evento}</Text></Text>
              <Text style={styles.eventosDescricao}>Hora:  <Text style={styles.dados}>{eventos.Hora_do_evento}</Text></Text>
              <Text style={styles.eventosDC}>Local:  <Text style={styles.dados}>{eventos.Local_do_evento}</Text></Text>

              <Text style={styles.eventosResponsavel}>Responsável:  <Text style={styles.dados}>{eventos.Responsavel_id}</Text></Text>
              <Text style={styles.eventosDescricao}>Descrição:  <Text style={styles.dados}>{eventos.Descricao}</Text></Text>

              
            </View>
          
        
    



            <TouchableOpacity
                style={styles.detailsButton}
                onPress={navigateToDetail}>
                <Text style={styles.detailsButtonText}> Voltar aos Eventos</Text>
                <Icone name="arrow-right" size={16} color={AMB.colorDefault}></Icone>
              </TouchableOpacity>


    
    </View>
    
    
  </View>);
}

